...
Ad_week = df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~  treatment + period + week+ base_bviral+ base_sviral, family = binomial, data = _)
...
### exclude week
Ad_noweek = df |> glmmTMB(cbind(7-Ad,Ad) ~  treatment + period+ base_bviral+ base_sviral, family = binomial, data = _)
...
Fit model with the assumption that `7-Ad` follows zero-inflation binomial


```{r}
Ad_week = df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~  treatment + period + week+ base_bviral+ base_sviral, ziformula = ~1, family = binomial, data = _)
...
### exclude week
Ad_noweek = df |> glmmTMB(cbind(7-Ad,Ad) ~  treatment + period+ base_bviral+ base_sviral,  ziformula = ~1, family = binomial, data = _)
...
The binomial model with zero-inflation for `7-Ad` is better than the binomial for `7-Ad` without zero inflation.  So we choose to use the following model with week being treated as a continuous variable.
...
Ad_mod =  dem_df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~  treatment + period + week, ziformula = ~1, family = binomial, data = _)
...
Ad_mod =  dem_df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~ product + regimen + age + gender + race + base_bviral + week +period + (1|ptid), data =_, ziformula = ~1, family = binomial)
summary(Ad_mod)
```
...
bv_mod_ap =  dem_df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~ age*product + regimen + base_bviral + week +period + (1|ptid), data =_, ziformula = ~1, family = binomial)

bv_mod_ar =  dem_df |> mutate(week = as.numeric(week)) |> 
  glmmTMB(cbind(7-Ad,Ad) ~ product + age*regimen + base_bviral + week +period + (1|ptid), data =_, ziformula = ~1, family = binomial)
